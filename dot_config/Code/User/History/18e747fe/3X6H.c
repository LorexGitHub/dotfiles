...
void sendTimeRequest(int soc, const time_request_t* timeRequest) {
  void sendTimeRequest(int soc, const time_request_t* timeRequest) {
    // Überprüfen, ob die Verbindung zum Server offen ist
    if (soc != -1) {
        // Datenpakete erstellen
        size_t dataSize = sizeof(time_request_t);
        char buffer[dataSize];
        memcpy(buffer, timeRequest, dataSize);

        // Daten an den Server senden
        ssize_t bytesSent = send(soc, buffer, dataSize, 0);
        if (bytesSent == -1) {
            // Fehler beim Senden der Daten
            perror("send");
        }
    } else {
        // Fehler: Die Verbindung zum Server ist nicht offen
        perror("connect");
    }
}
}
